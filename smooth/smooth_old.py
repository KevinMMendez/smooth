...
def csaps_fit(x, y, p=-1):
    ...
    if n == 2:
        ...
        if p < 0:
            ...
            p = 1 / (1 + (np.trace(R.toarray())) / (6 * np.trace(QtWQ)))
            a = 6 * (1 - p) * QtWQ + p * R
            b = np.diff(divdif)
            u = np.linalg.lstsq(a, b, rcond=None)[0]
        else:
            a = (6 * (1 - p) * (np.matmul(Qtw.toarray().T, Qtw.toarray()))) + p * R
            b = np.diff(divdif)
            u = np.linalg.lstsq(a, b, rcond=None)[0]
...
def csaps_predict(output, xx):
    # Put into bins
    newoutput = output["breaks"][1:-1]

    if len(newoutput) == 0:
        # Linear regression as there are two data points
        newx = xx - output["breaks"][0]
        c = output["coefs"][1]
        m = output["coefs"][0]
        val = m * newx + c
    else:
        idx = np.digitize(xx, newoutput, right=False)

        # get initial newx
        newx = xx - output["breaks"][idx]
